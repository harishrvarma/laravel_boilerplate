<?php
namespace Modules\Core\View\Components\Eav\Listing;

use Exception;
use Modules\Core\View\Components\Eav\Block;
use Illuminate\Pagination\LengthAwarePaginator;
use Modules\Core\View\Components\Eav\Listing\Grid\Pagination;
...
class Grid extends Block
{
    ...
    protected $filterFormat = [
        'name'=> null,
        'type'=> 'text',
        'filterClassName'=> '\Modules\Core\View\Components\Eav\Listing\Grid\Filter',
        'template' => null,
        'column'   => null,
    ];
    ...
    protected function applyFilters($query)
    {
        ...
        foreach ($this->filters() as $colName => $filter) {
            ...
            switch ($type) {
                ...
                case 'range':
                    $min = $filters[$column]['min'] ?? null;
                    $max = $filters[$column]['max'] ?? null;
                    if ($min !== null && $min !== '') {
    ...
    protected function handleHiddenColumns($primaryKey = 'id', $attributes = null)
    {
        $module = $this->gridKey();
        ...
        if (request()->has('columns')) {
            ...
            $hiddenColumns = session($sessionKey);
        } 
        else {
            $hiddenColumns = [];
            ...
            if ($attributes) {
                foreach ($attributes as $attr) {
                    if ($attr->config?->show_in_grid && !$attr->config?->default_in_grid) {
                        $hiddenColumns[] = $attr->code;
                    }
                }
            }
            session([$sessionKey => $hiddenColumns]);
